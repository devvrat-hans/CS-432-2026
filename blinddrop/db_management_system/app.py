...
try:
    ...
    from assignment03.Module_B.db_management_system.cleanup import start_cleanup_daemon
...
# CORS: allow frontend origin for public endpoints.
# In production, set CORS_ORIGIN to your Vercel domain (e.g. https://blinddrop.vercel.app)
_cors_origin = os.environ.get("CORS_ORIGIN", "http://localhost:3000")
_cors_origins = [_cors_origin, "http://127.0.0.1:3000"]
CORS(
    app,
    resources={
        r"/api/public/*": {"origins": _cors_origins},
...
def add_security_headers(response):
    ...
    # No caching for download responses and public API endpoints.
    if flask_request.path.startswith("/api/public/"):
...
# Start cleanup daemon on module load (works with both gunicorn and direct execution)
