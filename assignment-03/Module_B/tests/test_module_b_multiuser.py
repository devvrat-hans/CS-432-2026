...
class TestModuleBMultiUserBehavior(unittest.TestCase):
    ...
    def _create_regular_user_and_token(self, admin_token):
        username = f"multiuser_{secrets.token_hex(6)}"
        ...
        with app.test_client() as client:
            create_response = client.post(
                ...
                json={
                    "username": username,
                    "password": password,
                    "full_name": "Module B Multiuser",
                    "email": f"{username}@blinddrop.local",
                    "role": "user",
                    "member_group": "module-b",
                },
            )
    ...
    def test_parallel_admin_user_sessions_enforce_rbac_isolation(self):
        admin_token = self._admin_token()
        user_token = self._create_regular_user_and_token(admin_token)

        db_name = f"rbac_db_{secrets.token_hex(4)}"
        table_name = f"rbac_table_{secrets.token_hex(4)}"
        record_id = "shared"
        ...
        with app.test_client() as client:
            ...
            self.assertEqual(
                client.post(
                    f"/api/databases/{db_name}/tables/{table_name}/records",
                    headers=self._headers(admin_token),
                    json={"id": record_id, "payload": "seed"},
                ).status_code,
                201,
            )
        ...
        def admin_update_worker(index):
            with app.test_client() as client:
                response = client.put(
                    f"/api/databases/{db_name}/tables/{table_name}/records/{record_id}",
                    headers=self._headers(admin_token),
                    json={"payload": f"admin-update-{index}"},
                )
            return response.status_code

        def user_delete_worker(_):
            with app.test_client() as client:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
            admin_futures = [executor.submit(admin_update_worker, index) for index in range(admin_requests)]
            user_futures = [executor.submit(user_delete_worker, index) for index in range(user_requests)]
            admin_statuses = [future.result() for future in admin_futures]
            user_statuses = [future.result() for future in user_futures]

        self.assertTrue(all(status == 200 for status in admin_statuses), admin_statuses)
        self.assertTrue(all(status == 403 for status in user_statuses), user_statuses)
        ...
        self.assertEqual(records[0]["data"].get("id"), record_id)
